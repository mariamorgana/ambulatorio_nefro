  ...
  return (
    <div className="app">
      ...
      <main className="content">
        ...
        {tab === "data" && <DateSearch days={days} absenceByDate={absenceByDate} />}
      </main>
...
/* ================= DATE SEARCH ================= */
function DateSearch({ days, absenceByDate }) {
  ...
  const pct = dayInfo ? Math.min(100, Math.round((dayInfo.marcados / LIMIAR_OVERFLOW) * 100)) : 0;
    ...
    <section className="search-section">
      ...
      {!selectedDate && (
        <div className="empty-state">
          ...
          <p>Escolha uma data para ver a ocupação da agenda naquele dia.</p>
        </div>
      )}
      ...
      {selectedDate && !dayInfo && (
        <div className="empty-state">
          <Icon.Alert size={28}/>
          <p>Nenhum dado de agenda encontrado para esta data.</p>
        </div>
      )}
      ...
      {selectedDate && dayInfo && (
        <div className="patient-card">
          ...
          <div className="day-meta" style={{ marginBottom: 12 }}>
            {dayInfo.livres > 0 && <span className="meta-chip">{dayInfo.livres} vaga{dayInfo.livres>1?"s":""} livre{dayInfo.livres>1?"s":""}</span>}
            {dayInfo.reservado > 0 && <span className="meta-chip">{dayInfo.reservado} reservada{dayInfo.reservado>1?"s":""}</span>}
            {dayInfo.feriado > 0 && <span className="meta-chip">{dayInfo.feriado} bloqueio(feriado)</span>}
            {dayInfo.bloqueio_outros > 0 && <span className="meta-chip">{dayInfo.bloqueio_outros} bloqueio(outros)</span>}
          </div>
